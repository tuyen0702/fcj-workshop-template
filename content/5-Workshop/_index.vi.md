...
Ở giai đoạn xử lý AI, hệ thống sử dụng AWS Lambda Diagram Extractor để đọc sơ đồ đã upload từ Amazon S3 Input Bucket. Lambda Diagram Extractor gửi diagram sang Amazon Bedrock để Bedrock nhận diện các AWS services, connections, text notes và sinh ra architecture JSON có cấu trúc. Sau đó, architecture JSON được gửi sang AWS Lambda Cost Tool để ước tính chi phí hàng tháng. Kết quả ước tính chi phí cùng với architecture JSON tiếp tục được gửi lại sang Amazon Bedrock để đánh giá tổng thể kiến trúc theo AWS Well-Architected Framework, giải thích chi phí và đề xuất tối ưu.

Sau khi hoàn tất phân tích, AWS Lambda PDF Generator tạo báo cáo PDF từ kết quả đánh giá cuối cùng, lưu báo cáo vào Amazon S3 Report Bucket, cập nhật review history trong Amazon DynamoDB và gửi email notification cho người dùng thông qua Amazon SNS. Amazon CloudWatch được sử dụng để ghi log, giám sát và hỗ trợ troubleshooting. AWS IAM được sử dụng để kiểm soát quyền truy cập giữa các dịch vụ theo nguyên tắc least privilege access.
...
Sau khi file diagram được lưu vào S3 Input Bucket, Amazon S3 phát sinh Object Created Event. Event này được gửi đến Amazon EventBridge. EventBridge sử dụng rule đã cấu hình để khởi động AWS Step Functions Review Workflow. Step Functions chịu trách nhiệm điều phối toàn bộ quá trình xử lý review, bao gồm extraction, cost estimation, AI review, PDF generation, lưu kết quả và gửi thông báo.

Trong Processing Layer, Step Functions gọi AWS Lambda Diagram Extractor. Lambda Diagram Extractor đọc uploaded diagram từ S3 Input Bucket, sau đó gửi diagram sang Amazon Bedrock. Ở bước này, Amazon Bedrock đóng vai trò phân tích sơ đồ ban đầu để nhận diện các AWS services, connections, text notes và các thành phần kiến trúc. Kết quả trả về là architecture JSON có cấu trúc.

Sau khi có architecture JSON, Lambda Diagram Extractor gửi dữ liệu này sang AWS Lambda Cost Tool. Lambda Cost Tool phân tích danh sách AWS services được phát hiện trong architecture JSON và ước tính monthly cost dựa trên các giả định sử dụng ở mức demo. Kết quả cost estimation bao gồm danh sách dịch vụ, giả định sử dụng, chi phí ước tính và các thành phần có khả năng ảnh hưởng lớn đến ngân sách.
...
Sau khi final AI review hoàn tất, AWS Lambda PDF Generator tạo báo cáo PDF từ kết quả phân tích. Báo cáo được lưu vào Amazon S3 Report Bucket. Lambda PDF Generator cũng cập nhật review history và review status vào Amazon DynamoDB. Cuối cùng, Amazon SNS gửi email notification cho người dùng để thông báo rằng quá trình review đã hoàn tất.
...
8. AWS Step Functions điều phối workflow và gọi AWS Lambda Diagram Extractor để xử lý diagram.
...
16. Amazon CloudWatch ghi logs, metrics và hỗ trợ troubleshooting cho Lambda, API Gateway, Step Functions và workflow execution.
17. AWS IAM áp dụng least privilege access giữa các services trong toàn bộ hệ thống.
...
- **AWS Lambda Diagram Extractor**: Đọc uploaded diagram từ S3 Input Bucket và gửi diagram sang Amazon Bedrock để sinh architecture JSON.
...
- **Amazon SNS**: Gửi email notification cho người dùng khi quá trình review hoàn tất.
- **Amazon CloudWatch**: Ghi logs, theo dõi metrics và hỗ trợ troubleshooting cho Lambda, API Gateway và Step Functions.
...
2. **Xây dựng Lambda Diagram Extractor**
   - Tạo AWS Lambda Diagram Extractor.
...
4. **Xây dựng Lambda Cost Tool**
   ...
   - Nhận architecture JSON từ Lambda Diagram Extractor.
...
7. **Gửi Notification**
   - Cấu hình Amazon SNS topic.
   - Thêm email subscription cho SNS topic.
   - Xác nhận email subscription.
   - Gửi email notification khi quá trình review hoàn tất.
   - Nội dung email notification nên bao gồm:
     - Review ID.
     - Review status.
     - Thời gian hoàn tất.
     - Tóm tắt kết quả review.
     - Thông tin report PDF.
   - Trường hợp workflow thất bại, gửi notification hoặc cập nhật trạng thái failed để người dùng có thể kiểm tra trên frontend.
...
- Lambda Diagram Extractor có thể đọc uploaded diagram và gửi diagram sang Amazon Bedrock.
...
- Amazon SNS gửi email notification khi review hoàn tất.
- CloudWatch ghi logs và hỗ trợ troubleshooting.
- IAM permissions được kiểm tra theo nguyên tắc least privilege access.
- Hệ thống có thể chạy end-to-end từ upload diagram đến nhận report và notification.
...
7. [Tạo PDF Report và gửi Notification](5.7-Reporting-notification/)
8. [Monitoring, Security và IAM Least Privilege](5.8-Monitoring-security/)
